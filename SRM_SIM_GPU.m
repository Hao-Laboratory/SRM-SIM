...
[Y,X] = ndgrid(1:M,1:N);
rad = gpuArray(hypot(X-floor(N/2+1),Y-floor(M/2+1)));
...
pattern = imresize(double(imReadStackGPU([dataDir,patternName])),scaleFactor,'Method','bilinear');
...
shiftedEffOTF = gpuArray(zeros(size(notchedEffOTF)));
kr = gpuArray(zeros(nDir,1));
for iDir = 1:nDir
...
apoFunSIM = gpuArray(cos(pi*rad/(2*kcSIM))).*attFunPost;  
...
sample = imresize(double(imReadStackGPU([dataDir,sampleName])),scaleFactor,'Method','bilinear');
